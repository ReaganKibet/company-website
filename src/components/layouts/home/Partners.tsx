...
const Partners = ({ onSectionChange }: PartnersProps) => {
  ...
  return (
    <section 
      ref={sectionRef}
      className="relative py-20 overflow-hidden bg-transparent transition-colors duration-500"
    >
      <div className="relative z-10 w-full max-w-7xl mx-auto px-4">
        ...
        >
          {partners.map((partner, index) => (
            <div key={index} className="rounded-2xl w-32 h-32 flex items-center justify-center hover:shadow-md transition-all duration-300 hover:-translate-y-1 bg-card border border-border p-4">
              <img src={partner} alt={`Partner ${index + 1}`} className="max-w-full max-h-full object-contain rounded-2xl dark:brightness-200 dark:grayscale dark:contrast-150" />
            </div>
          ))}
        </div>
      </div>
